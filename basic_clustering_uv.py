...
def reshape_sigma(s):
  ...
  if s.size==1:
    if s.shape == (): s = np.array([s])
    s = np.tile(s[:,None], (w.size, ndims))
  else:
    s = np.reshape(s, (w.size, ndims))
...
def sample_cluster_likelihoods(model, nclusters_fit, data, trace):
  ...
  with model:
    ...
    pps = pm.sample_posterior_predictive(trace, var_names=["cluster_likelihoods"])

  return pps


def compute_cluster_likelihoods(data, model, trace=None, MAP=None):
  ...
  if trace is not None:
    ...
    σs = az.summary(trace, var_names="σs").iloc[:,0].values
  ...
  with pm.Model():
    components = [pm.Normal.dist(mus[i,:],  σs[i,:]) for i
...
def plot(cluster_likelihoods, colors, centers, data, max_len=None,
         dim0=0, dim1=1):
...
def bayesian_clustering(data, nclusters_fit, conc=1, mu_sigma=2,
                        est_centers=None, sample=False):

  if est_centers is None:
    init0 = np.linspace(-max_len, max_len, nclusters_fit)
    init1 = None
    mu0 = data.mean(axis=0)[0]
    mu1 = data.mean(axis=0)[1:]
  else:
    init0 = est_centers[:,0]
    init1 = est_centers[:,1:]
    mu0 = est_centers[:,0]
  ...
  with pm.Model() as model:
    ...
    var = pm.Exponential("var", 1)
    σ = pm.LogNormal("σ", mu, var)
    ...
    if sample:
      trace = pm.sample(draws=2000, chains=6, tune=2000, target_accept=0.95)
    else:
      MAP = pm.find_MAP()
...
data, centers, labels_true = generate_data(ndims, nclusters, max_len,
                                           sd, ndata, seed=0)
...
if not sample:
  ...
  mus = np.reshape(az.summary(trace, var_names="μ").iloc[:,0].values,
                   (nclusters, ndims))
  w = az.summary(trace, var_names="w").iloc[:,0].values
  s = az.summary(trace, var_names="σ").iloc[:,0].values
...
colors = np.array([[1,0,0],[0,0,1],[0,1,0],[1,1,0], [0,0,0], [1,1,1]])[:nclusters_fit,:]
